...
export function createRouter(root) {
  ...
  function render(page = current) {
    ...
    const view = pageViews[current];
    root.innerHTML = `${Navbar(current)}<main><section class="page-head"><p class="eyebrow">${meta.label}</p><h2>${meta.title}</h2><p>${meta.intro}</p></section><section class="page-content">${view()}</section></main>${Footer()}`;
    bind();
  ...
  function bind() {
    ...
    root.querySelectorAll('.flow-step').forEach((btn) => btn.addEventListener('click', () => {
      ...
      const [name, description] = (await import('./data.js')).loop[index];
